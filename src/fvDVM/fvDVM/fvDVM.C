...
void Foam::fvDVM::updateMacroVol()
{
    ...
    volScalarField rhoEvol = 0.5*rhoold*(magSqr(Uold) + (KInner_ + 3)* R_* Told);
    ...
    rhoVol_ = rhoold - rhoVol_;

    Uvol_ = (rhoold*Uold - rhoUvol) / rhoVol_;

    Tvol_ = ((rhoUUvol - rhoEvol) - 0.5*rhoVol_*magSqr(Uvol_))/((KInner_ + 3)/2.0*R_*rhoVol_);
    ...
    forAll(DV_, dvi)
    {
        ...
        stress_ +=
            dXiCellSize_*dv.weight()*dv.gVol()*c*c;
    }
